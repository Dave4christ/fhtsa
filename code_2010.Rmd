...
```{r}
intersect(
  colnames(accident_2010), colnames(person_2010)
)
accident_2010 %>% 
  left_join(
    person_2010, by = c(
      "state", "st_case", "ve_forms", "county", "day", "month",
      "hour", "minute", "harm_ev", "man_coll", "sch_bus")
  ) -> accident_person_2010
```
```{r}
intersect(
  colnames(accident_person_2010), colnames(vehicle_2010)
)
accident_person_2010 %>% 
  left_join(
    vehicle_2010, by = c(
...
```
```{r}
save(apv_2010, file = "Data/FARS2010/apv_2010.RData")
```
...
```
```{r}
save(apv_2010_2, file = "Data/FARS2010/apv_2010_2.RData")
```
```{r}
apv_2010_2 %>%
group_by(harm_ev) %>%
...
apv_2010_2 %>% 
  select(
    -c(deformed, trav_sp, bus_use, haz_inv, haz_rel, hit_run, unittype,
       location, hispanic, reg_stat, owner, model, underide,
       rolinloc, towed, m_harm, rail)
  ) -> apv_2010_2
```
```{r}
apv_2010_2 %>% mutate(
...
apv_2010_4 %>% 
  select(
    -c(reljct1, rel_road, wrk_zone, weather, cf1, cf2, cf3, n_mot_no,
       road_fnc, impact2, veh_sc1, veh_sc2, prev_sus, dr_sf1, dr_sf2,
       dr_sf3, dr_sf4,)
  ) %>% mutate(
    race = case_when(
      race == 0 ~ "Not a Fatality(Not Applicable)",
      race == 1 ~ "White",
      race == 99 ~ "Unknown",
      race == 2 ~ "Black",
...
apv_2010_4 %>% 
  select(
    -c(last_yr, mod_year, vintype, veh_make,)
  ) -> apv_2010_4
```

```{r}
save(apv_2010_4, file = "Data/FARS2010/apv_2010_4.RData")
```
